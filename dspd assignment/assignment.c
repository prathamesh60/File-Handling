...
location *ReadNextFromFile(location *nptr,FILE *pFile)
{ size_t returnvalue;
  if(nptr==NULL)
   {
   	 nptr=malloc(sizeof(location));
   	 //nptr->res_menu=malloc(sizeof(menu));
   	 /*int x;
   	 fscanf(tFile,"%d",&x);
   	 nptr->res_menu->no_of_items=x;
   	  int i;
	  for(i=0;i<nptr->res_menu->no_of_items;i++)
		    {
		    	fscanf(tFile,"%s",&(nptr->res_menu->item[i].itemname));
		    	fscanf(tFile,"%f",&(nptr->res_menu->item[i].price));
		    }*/
   	 returnvalue=fread(nptr,sizeof(location),1,pFile);
   	 nptr->next=NULL;
   }
  else
   {
   	 location *index=nptr;
   	 location *new_car=malloc(sizeof(location));
   	 while(index->next!=NULL)
   	  { index=index->next;
	  }
     returnvalue=fread(new_car,sizeof(location),1,pFile);
     index->next=new_car;
     new_car->next=NULL;
   }
 return nptr;  
}
location  *load_database(cat_location **res,cat_location **pub,cat_location **cafe,cuis_location **north,cuis_location **south,cuis_location **cont)
{ FILE *pFile;
 // pFile=fopen("test.bin","rb");
  ...
  if(pFile!=NULL)
   { location *kptr=NULL;
   	 ...
   	 rewind(pFile);
   	 //rewind(tFile);
   	 int numEntries=(int)(filesize/sizeof(location));
   	// printf("NO Of Entries: %d\n",numEntries);
   	 int i;
   	 for(i=0;i<numEntries;i++)
   	  { 
   	    fseek(pFile,(sizeof(location)*i),SEEK_SET);
   	    kptr=ReadNextFromFile(kptr,pFile);
   	    
	  }
	fclose(pFile);
//	fclose(tFile);
    FILE *tFile;
    tFile=fopen("menu.txt","r");
    fseek(tFile,0,SEEK_END);
    rewind(tFile);
	location *nptr=kptr;
	 while(nptr!=NULL)
	  {
        nptr->res_menu=malloc(sizeof(menu));
        fscanf(tFile,"%d",&(nptr->res_menu->no_of_items));
        int i;
        for(i=0;i<nptr->res_menu->no_of_items;i++)
		    {
		    	fscanf(tFile,"%s\n",nptr->res_menu->item[i].itemname);
		    	fscanf(tFile,"%f\n",&(nptr->res_menu->item[i].price));
		    }
			if(nptr->category==1)
	        {
		      insert_cat_location(res,nptr->name,nptr->address);
	        } 
	        else if(nptr->category==2)
	        {
		      insert_cat_location(cafe,nptr->name,nptr->address);
	        } 
	        else
	        {
		      insert_cat_location(pub,nptr->name,nptr->address);
	        } 
         	if(nptr->cuis_category==1)
        	{
		       insert_cuis_location(north,nptr->name,nptr->address);
          	}
            else if(nptr->cuis_category==2)
         	{
               	insert_cuis_location(south,nptr->name,nptr->address);
        	}
        	else
        	{
	        	insert_cuis_location(cont,nptr->name,nptr->address);
	        }
    	nptr=nptr->next;
	  }
	  fclose(tFile);
...
{ 
   ...
   tFile=fopen("menu.txt","w");
   if(pFile!=NULL && tFile!=NULL)
    { rewind(pFile);
      rewind(tFile);
	  location *nptr=all_eatspots,*ptr;
      while(nptr!=NULL)
	   { ptr=nptr->next;
	     nptr->next=NULL;
	     fwrite(nptr,sizeof(location),1,pFile);
	     fprintf(tFile,"%d\n",nptr->res_menu->no_of_items);
	     int i;
		 for(i=0;i<nptr->res_menu->no_of_items;i++)
		 {
		    fprintf(tFile,"%s\n",nptr->res_menu->item[i].itemname);
...
{   statuscode sc;
	...
	all_eatspots=load_database(&res,&pub,&cafe,&north,&south,&cont);
   ...
   // res_no_of_seats=100;
    ...
    while(query!=0 && flag==0)
    {
    	...
    	printf("Press <18> to print list of all eating spots:\n");
    	scanf("%d",&query);
    	switch(query)
		{
    		case 0:flag=1;break;
            ...
            case 18:Traverse(all_eatspots);break;
			default:printf("Please enter appropriate choice");
        			break;
    	}
       
	}    
	Reinsert_into_database(all_eatspots);
   return 0;   
}
